...
from src.models import MarketAgentOutput, StartupIdea, UserConstraints, llm_model
from src.utils import parse_json_from_llm

def market_node(startup_idea: StartupIdea, constraints: UserConstraints) -> MarketAgentOutput:
    ...
    json_instruction = (
        "Return ONLY JSON with keys: customer_demand_curve (list of floats), "
        "competitors_reactions (list of strings), opportunities_identified (list of strings)."
    )
    messages = [
        SystemMessage(content=StartupToolPrompts.SYSTEM_MARKET_ANALYSIS),
        HumanMessage(
            content=StartupToolPrompts.market_analysis_user(startup_text, constraints_text)
            + "\n\n"
            + json_instruction
        ),
    ]
    ...
    return MarketAgentOutput(**parsed)