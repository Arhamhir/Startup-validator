...
from src.models import LegalAgentOutput, StartupIdea, UserConstraints, llm_model
from src.prompts import StartupToolPrompts
from src.utils import parse_json_from_llm

def legal_node(startup_idea: StartupIdea, constraints: UserConstraints) -> LegalAgentOutput: 
    """Analyzes legal and regulatory considerations and provides structured output"""
    llm = llm_model()
    ...
    json_instruction = (
        "Return ONLY JSON with keys: regulatory_risks (list of strings), "
        "mitigation_strategies (list of strings), compliance_requirements (object of bools)."
    )
    messages = [
        SystemMessage(content=StartupToolPrompts.SYSTEM_LEGAL_ANALYSIS),
        HumanMessage(
            content=StartupToolPrompts.legal_analysis_user(startup_text, constraints_text)
            + "\n\n"
            + json_instruction
        ),
    ]
    ...
    return LegalAgentOutput(**parsed)